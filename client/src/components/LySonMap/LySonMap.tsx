...
const LySonMap = ({ closeMap }: LySonMapProps) => {
  ...
  const places = [
    ...
    {
      className: "trung_bay",
      label: "Nhà Trưng bày Hải đội Hoàng Sa kiêm quản Bắc Hải",
      img: image.trung_bay,
      place: "trung_bay",
      number: 4,
    },
    ...
    <>
      <div className={`backgroundLS ${isOpen ? "blur" : ""}`}>
        {places.map(({ className, label, img, place, number }) => (
          <div
            key={place}
            className={`${className} image_LSMap`}
            onClick={() => displayBoxInfo(place, number)}
          >
            <Image src={img} alt="" />
            <p>{label}</p>
          </div>
